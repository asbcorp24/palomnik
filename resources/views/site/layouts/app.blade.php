            ...
            <div class="collapse navbar-collapse" id="siteNavigation">
                ...
                <div class="d-flex align-items-center gap-2">
                    @auth
                        ...
                        <div class="dropdown">
                            <button class="btn btn-outline-pm btn-sm dropdown-toggle d-flex align-items-center gap-2" data-bs-toggle="dropdown" type="button">
                                @if(auth()->user()->avatar_url)<img src="{{ auth()->user()->avatar_url }}" alt="" style="width:24px;height:24px;object-fit:cover;border-radius:50%">@else<i class="bi bi-person-circle"></i>@endif
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-4 p-2">
                                ...
                                @if(auth()->user()->canManageObjects())<li><a class="dropdown-item rounded-3" href="{{ route('service.dashboard') }}"><i class="bi bi-building-check me-2"></i>Кабинет представителя</a></li>@endif
...
<footer class="site-footer py-5">
    <div class="container">
        ...
        <div class="d-flex flex-wrap justify-content-between gap-3 small opacity-75"><span>© {{ date('Y') }} Московский паломник</span><span class="d-flex flex-wrap gap-3"><a class="footer-link" href="{{ route('privacy') }}">Персональные данные</a><a class="footer-link" href="{{ route('terms') }}">Правила сервиса</a></span></div>
    </div>
</footer>
...
<nav class="mobile-bottom-nav" aria-label="Мобильная навигация">
    ...
    <a class="{{ request()->routeIs('profile.*') || request()->routeIs('route-plans.*') || request()->routeIs('notifications.*') ? 'active' : '' }} position-relative" href="{{ auth()->check() ? route('profile.dashboard') : route('login') }}"><i class="bi bi-person"></i><span>Профиль</span>@if($unreadNotificationCount)<span class="position-absolute top-0 end-0 badge rounded-pill bg-danger">{{ $unreadNotificationCount > 9 ? '9+' : $unreadNotificationCount }}</span>@endif</a>
</nav>
