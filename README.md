...
- публичная Bootstrap-главная в стиле мобильного приложения;
- адаптивная верхняя и мобильная нижняя навигация;
- публичный каталог храмов и святынь;
- поиск, фильтры и сортировка объектов;
- подробная карточка объекта с историей, святынями, расписанием, контактами и галереей;
- интерактивная карта опубликованных объектов;
- предварительная страница будущих паломнических маршрутов;
...
/routes            предварительный раздел маршрутов
```

## Яндекс Карты

Для отображения интерактивной карты укажите в `.env` ключ JavaScript API:
...
Без ключа страница `/map` показывает понятный экран настройки, а каталог сайта продолжает работать.
