...
class _ObjectDetailScreenState extends State<ObjectDetailScreen> {
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Расскажите о храме или святыне',
                ),
              ),
            ],
          ),
  ...
  void _snack(String message, {bool error = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: error ? Colors.red : AppTheme.green,
      ),
    );
  }
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed: _busy ? null : () => _visit(item),
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (_text(data['description']) != null) ...[
                  ...
                  if (_text(amenities['parking']) != null)
                    FeatureSection(
                      title: 'Парковка',
                      text: '${amenities['parking']}',
                      icon: Icons.local_parking,
                    ),
                  if (_text(amenities['accessibility']) != null)
                    FeatureSection(
                      title: 'Доступность',
                      text: '${amenities['accessibility']}',
                    ...
                    Card(
                      child: Column(
                        children: [
                          ...
                          if (_text(contacts['email']) != null)
                            ListTile(
                              leading: const Icon(Icons.email),
                              title: Text('${contacts['email']}'),
                              onTap: () => launchUrl(
                                Uri.parse('mailto:${contacts['email']}'),
                              ),
                            ),
  ...
  Widget _placeholder() => const SizedBox(
    ...
    child: ColoredBox(
      ...
      child: Center(child: Icon(Icons.church, size: 70, color: AppTheme.gold)),
    ),
  );
}
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${response.data['message'] ?? 'Бронирование создано.'} Код: ${response.data['ticket_code'] ?? ''}',
            ),
          ),
        );
    } catch (error) {
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                Chip(
                  label: Text(
                    '${item['difficulty'] ?? 'Сложность не указана'}',
                  ),
                ),
                ...
                return Card(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          formatFeatureDate(trip['starts_at']),
                          style: Theme.of(context).textTheme.titleMedium
                              ?.copyWith(fontWeight: FontWeight.w700),
                        ),
                        const SizedBox(height: 6),
                        Text(
                          '${trip['meeting_point'] ?? 'Место встречи уточняется'}',
                        ),
                        const SizedBox(height: 8),
                        Text(
                          'Цена: ${trip['price'] ?? item['base_price'] ?? 0} ₽ · занято ${trip['booked_count'] ?? 0}/${trip['capacity'] ?? '∞'}',
                        ),
                        const SizedBox(height: 12),
                        FilledButton.icon(
                          onPressed: isOpen ? () => _book(trip) : null,
                          icon: const Icon(Icons.confirmation_number_outlined),
                          label: Text(
                            isOpen ? 'Забронировать' : 'Запись закрыта',
                          ),
                        ),
                      ],
                    ),
        ...
        return ListView(
          ...
          children: [
            ...
            FeatureInfo(
              ...
              text: formatFeatureDate(item['starts_at']),
            ),
            if (_text(item['location'] ?? item['address']) != null)
              FeatureInfo(
            ...
            Wrap(
              ...
              runSpacing: 10,
              children: [
                if (item['ics_url'] != null)
                  FilledButton.icon(
                    onPressed: () => launchUrl(
                      Uri.parse('${item['ics_url']}'),
                      mode: LaunchMode.externalApplication,
                    ),
                    icon: const Icon(Icons.event_available),
                    label: const Text('В календарь'),
                  ),
                if (item['registration_url'] != null)
                  OutlinedButton.icon(
                    onPressed: () => launchUrl(
                      Uri.parse('${item['registration_url']}'),
                      mode: LaunchMode.externalApplication,
                    ),
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            Text(
              '${author['name'] ?? 'Паломник'} · ${formatFeatureDate(item['published_at'])}',
              style: const TextStyle(color: Colors.black54),
            ),
            if (media.isNotEmpty) ...[
              const SizedBox(height: 18),
              ...media
                  .where((value) => _map(value)['type'] == 'image')
                  .map(
                    (value) => Padding(
...
class _TogetherDetailScreenState extends State<TogetherDetailScreen> {
  ...
  Future<void> _join() async {
    try {
      final response = await ApiClient.instance.dio.post(
        '/mobile/together/${widget.slug}/join',
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Статус заявки: ${response.data['status']}')),
        );
  ...
  Future<void> _leave() async {
    try {
      await ApiClient.instance.dio.delete(
        '/mobile/together/${widget.slug}/leave',
      );
      if (mounted) setState(() => _future = _load());
    } catch (error) {
      if (mounted)
  ...
  Future<void> _send() async {
    ...
    try {
      await ApiClient.instance.dio.post(
        '/mobile/together/${widget.slug}/messages',
        data: {'body': body},
      );
      _message.clear();
      if (mounted) setState(() => _future = _load());
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                formatFeatureDate(message['created_at']),
                                style: const TextStyle(
                                  fontSize: 11,
...
class _CreateTogetherScreenState extends State<CreateTogetherScreen> {
  ...
  Future<void> _save() async {
    if (_startsAt == null ||
        ...
        _meeting.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Заполните название, описание, дату и место встречи.'),
        ),
      );
      return;
    }
    ...
    body: ListView(
      ...
      children: [
        ...
        TextField(
          ...
          minLines: 5,
          maxLines: 10,
          decoration: const InputDecoration(labelText: 'Описание и план'),
        ),
        const SizedBox(height: 12),
        TextField(
          controller: _meeting,
        ...
        DropdownButtonFormField<String>(
          ...
          items: const [
            DropdownMenuItem(
              value: 'approval',
              child: Text('По заявке организатору'),
            ),
            DropdownMenuItem(
              value: 'auto',
              child: Text('Свободное присоединение'),
            ),
          ],
        ...
        FilledButton(
          ...
          child: Text(_busy ? 'Сохранение...' : 'Отправить на модерацию'),
        ),
      ],
    ),
  );
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: const [
            ...
            DropdownMenuItem(
              value: 'registered',
              child: Text('Пользователи сервиса'),
            ),
