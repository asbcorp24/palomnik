...
<head>
    ...
    <style>
        :root {
            --pilgrim-cream: #f7f0e6;
            --pilgrim-paper: #fffdf9;
            --pilgrim-gold: #b08a3e;
            --pilgrim-gold-dark: #8c6b2d;
            --pilgrim-green: #26443b;
            --pilgrim-green-soft: #355e52;
            --pilgrim-brown: #6f4d37;
            --pilgrim-ink: #25211d;
            --pilgrim-muted: #746c64;
            --pilgrim-border: rgba(111, 77, 55, .14);
        }
        body {
            min-height: 100vh;
            background: #f6f3ed;
            color: var(--pilgrim-ink);
        ...
        .admin-sidebar {
            ...
            background:
                ...
                linear-gradient(165deg, var(--pilgrim-green), #172d27 78%);
            box-shadow: 12px 0 45px rgba(25, 39, 34, .12);
        }
        .brand-block { padding: 25px 22px 20px; border-bottom: 1px solid rgba(255,255,255,.1); }
        .brand-mark {
            width: 46px; height: 46px; display: inline-flex; align-items: center; justify-content: center;
            border: 1px solid rgba(255,255,255,.28); border-radius: 50%; color: #eed9a1;
            background: rgba(255,255,255,.06); font-size: 1.35rem;
        }
        ...
        .admin-topbar {
            ...
            backdrop-filter: blur(14px); position: sticky; top: 0; z-index: 1020;
        }
        ...
        @media (max-width: 991.98px) {
            ...
            .admin-topbar, .admin-content { padding-left: 18px; padding-right: 18px; }
        }
    </style>
    @stack('styles')
</head>
...
<div class="admin-shell">
    <aside class="admin-sidebar">
        <div class="brand-block d-flex align-items-center gap-3">
            ...
            <div>
                <div class="brand-title">Московский паломник</div>
                <div class="brand-subtitle">Управление платформой</div>
            </div>
        </div>
        ...
        <nav class="sidebar-nav">
            ...
            <a class="sidebar-link {{ request()->is('admin/directories/object-types*') ? 'active' : '' }}" href="{{ route('admin.directories.index', 'object-types') }}">
                <i class="bi bi-pin-map"></i><span>Типы объектов</span>
            </a>
            <a class="sidebar-link {{ request()->is('admin/directories/vicariates*') ? 'active' : '' }}" href="{{ route('admin.directories.index', 'vicariates') }}">
                <i class="bi bi-diagram-3"></i><span>Викариатства</span>
            </a>
            <a class="sidebar-link {{ request()->is('admin/directories/deaneries*') ? 'active' : '' }}" href="{{ route('admin.directories.index', 'deaneries') }}">
                <i class="bi bi-building"></i><span>Благочиния</span>
            </a>
            <a class="sidebar-link {{ request()->is('admin/directories/sanctities*') ? 'active' : '' }}" href="{{ route('admin.directories.index', 'sanctities') }}">
                <i class="bi bi-star"></i><span>Святыни</span>
            </a>
        </nav>
    </aside>
    ...
    <main class="admin-main">
        <header class="admin-topbar">
            <div class="d-flex align-items-center gap-3">
                <button class="btn btn-light sidebar-toggle" type="button" onclick="document.body.classList.toggle('sidebar-open')">
                    <i class="bi bi-list"></i>
                </button>
                <div class="small text-secondary">Административная панель</div>
            </div>
            <div class="d-flex align-items-center gap-3">
                ...
                <form method="POST" action="{{ route('admin.logout') }}">
                    @csrf
                    <button class="btn btn-sm btn-outline-secondary" type="submit" title="Выйти">
